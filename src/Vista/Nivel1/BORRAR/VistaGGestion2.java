/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

/*
 * VistaGGestion.java
 *
 * Created on 23-nov-2011, 23:27:34
 */
package Vista.Nivel1;

import Vista.Modelos.GestionarModelo;
import Vista.Modelos.BORRAR.BaseTableModel;
import Vista.Nivel2.FichaGBase;
import Controlador.Controlador;
import Controlador.Evento;
import Controlador.GestorEventos;
import Controlador.TipoEvento;
import Vista.Modelos.BaseTableModelGen;
import java.awt.Color;
import java.awt.Component;
import java.awt.Graphics;
import java.awt.Polygon;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.beans.PropertyChangeEvent;
import java.util.Collection;
import java.util.List;
import javax.swing.AbstractCellEditor;
import javax.swing.DefaultListCellRenderer;
import javax.swing.Icon;
import javax.swing.JButton;
import javax.swing.JColorChooser;
import javax.swing.JLabel;
import javax.swing.JList;
import javax.swing.JTable;
import javax.swing.ListCellRenderer;
import javax.swing.event.TableModelEvent;
import javax.swing.event.TableModelListener;
import javax.swing.table.DefaultTableCellRenderer;
import javax.swing.table.TableCellEditor;
import javax.swing.table.TableCellRenderer;
import javax.swing.table.TableColumn;

/**
 *
 * @author nanohp
 */
public class VistaGGestion2 extends javax.swing.JInternalFrame implements GestionarModelo, GestorEventos {

    /*
     * 
     */
    /*
     * 
     */
    private FichaGBase fichaElemento;
    private Controlador controlador;
    boolean administrador;
    private ParametrizadorGestion parametrosGestion;
    //private BaseTableModel tableModel;
    private BaseTableModelGen tableModelGen;

    /** Creates new form VistaGGestion */
    public VistaGGestion2(Controlador controlador, ParametrizadorGestion parametrosGestion, FichaGBase fichaGBase, boolean administrador) {
        //this.listaCategoriasDewey = listaCategoriasDewey;
        this.fichaElemento = fichaGBase;
        this.parametrosGestion = parametrosGestion;
        this.controlador = controlador;
        this.administrador = administrador;
        tableModelGen = parametrosGestion.getBaseTableModelGen();

        initComponents();


        inicializaComponentesPropios();
        controlador.procesarEvento(new Evento(parametrosGestion.getConsultaGeneral(), null, this, TipoEvento.CONSULTA_GENERAL));
    }

    private void inicializaComponentesPropios() {

        // Inicializamos la tabla
        setTitle(parametrosGestion.getTitulo());

        tableModelGen = parametrosGestion.getBaseTableModelGen();

        jTableListado.setModel(parametrosGestion.getBaseTableModelGen());

        // Fijamos el columModel

       

        // Ahora el resto


        jTableListado.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);

        jTableListado.getModel().addTableModelListener(new MyTableModelListener(jTableListado));


        /* Controla la visibildad del boton para altas de titulo
         * depediendo de si el usuario es administrador o no
         */
        if (administrador) {
            jButtonNuevo.setVisible(true);
        } else {
            jButtonNuevo.setVisible(false);
        }
    }

    @Override
    public void setVisible(boolean visible) {
        super.setVisible(visible);
        ocultaBotonesAccion();
    }

    public void ocultaBotonesAccion() {
        try {
            jButtonMostrar.setVisible(false);
            jButtonModificar.setVisible(false);
            jButtonBaja.setVisible(false);
        } catch (NullPointerException e) {
            System.err.println("Error en ocultaBotonesAccion");
        }
    }

    @Override
    public BaseTableModelGen obtenerModeloGen() {
        return parametrosGestion.getBaseTableModelGen();
    }

    @Override
    public void limpiarModelo() {
        tableModelGen = null;
    }

    @Override
    public void setEditable(boolean esEditable) {
        tableModelGen.setEditable(esEditable);
    }

    @Override
    public void setModo(String modo) {
    }

    /** This method is called from within the constructor to
     * initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is
     * always regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">                          
    private void initComponents() {

        jScrollPane1 = new javax.swing.JScrollPane();
        jTableListado = new javax.swing.JTable();
        jButtonAceptar = new javax.swing.JButton();
        jButtonMostrar = new javax.swing.JButton();
        jButtonModificar = new javax.swing.JButton();
        jButtonBaja = new javax.swing.JButton();
        jButtonNuevo = new javax.swing.JButton();

        setClosable(true);
        setDefaultCloseOperation(javax.swing.WindowConstants.HIDE_ON_CLOSE);
        setIconifiable(true);
        setMaximizable(true);
        setResizable(true);
        setTitle("Gestión");
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/images/catalogo_20.png"))); // NOI18N

        jTableListado.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null}
            },
            new String [] {
                "Title 1", "Title 2", "Title 3", "Title 4"
            }
        ));
        jTableListado.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        jTableListado.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jTableListadoMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(jTableListado);

        jButtonAceptar.setText("Aceptar");
        jButtonAceptar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonAceptarActionPerformed(evt);
            }
        });

        jButtonMostrar.setText("Mostrar");
        jButtonMostrar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonMostrarActionPerformed(evt);
            }
        });

        jButtonModificar.setText("Modificar");
        jButtonModificar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonModificarActionPerformed(evt);
            }
        });

        jButtonBaja.setText("Baja");
        jButtonBaja.setToolTipText("");
        jButtonBaja.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonBajaActionPerformed(evt);
            }
        });

        jButtonNuevo.setText("Nuevo");
        jButtonNuevo.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonNuevoActionPerformed(evt);
            }
        });

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 639, Short.MAX_VALUE)
                        .addContainerGap())
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jButtonAceptar)
                        .addGap(18, 18, 18)
                        .addComponent(jButtonNuevo)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, 268, Short.MAX_VALUE)
                        .addComponent(jButtonMostrar)
                        .addGap(18, 18, 18)
                        .addComponent(jButtonBaja)
                        .addGap(18, 18, 18)
                        .addComponent(jButtonModificar)
                        .addGap(20, 20, 20))))
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 315, Short.MAX_VALUE)
                .addGap(12, 12, 12)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jButtonAceptar)
                    .addComponent(jButtonModificar)
                    .addComponent(jButtonBaja)
                    .addComponent(jButtonNuevo)
                    .addComponent(jButtonMostrar))
                .addContainerGap())
        );

        pack();
    }// </editor-fold>                        

    private void jButtonAceptarActionPerformed(java.awt.event.ActionEvent evt) {                                               
        setVisible(false);
    }                                              

    private void jTableListadoMouseClicked(java.awt.event.MouseEvent evt) {                                           
        int pos = jTableListado.getSelectedRow();
        boolean visible = pos > -1 ? true : false;
        jButtonMostrar.setVisible(visible);

        if (administrador) {
            jButtonModificar.setVisible(visible);
            jButtonBaja.setVisible(visible);
        }
    }                                          

    private void jButtonMostrarActionPerformed(java.awt.event.ActionEvent evt) {                                               

        int filaSeleccionada = jTableListado.getSelectedRow();
        if (filaSeleccionada > -1) {
            System.out.println("--:" + tableModelGen.getElemento(filaSeleccionada));
            System.out.println("es el indice: " + tableModelGen.getPos(tableModelGen.getElemento(filaSeleccionada)));
            fichaElemento.fijarModelo(tableModelGen.getElemento(filaSeleccionada));
            fichaElemento.setModo("mostrar");
            fichaElemento.setVisible(true);
        }

    }                                              

    private void jButtonModificarActionPerformed(java.awt.event.ActionEvent evt) {                                                 
        int filaSeleccionada = jTableListado.getSelectedRow();
        if (filaSeleccionada > -1) {
            System.out.println("--:" + tableModelGen.getElemento(filaSeleccionada));
            fichaElemento.fijarModelo(tableModelGen.getElemento(filaSeleccionada));
            fichaElemento.setModo("modificar");
            fichaElemento.setVisible(true);
        }
    }                                                

private void jButtonBajaActionPerformed(java.awt.event.ActionEvent evt) {                                            
    int filaSeleccionada = this.jTableListado.getSelectedRow();
    if (filaSeleccionada > -1) {
        System.out.println("--:" + tableModelGen.getElemento(filaSeleccionada));
        fichaElemento.fijarModelo(tableModelGen.getElemento(filaSeleccionada));
        fichaElemento.setModo("baja");
        fichaElemento.setVisible(true);
    }
    /*
    Object elemento = tableModel.getInfo(filaSeleccionada);
    String cadena = elemento.toString();
    String mensaje = "¿Desea borrar el título?";
    String tituloStr = "Borrado de xxxx: " + cadena;
    
    int respuetaInt = JOptionPane.showConfirmDialog(null, mensaje, tituloStr, JOptionPane.YES_NO_OPTION);
    if (respuetaInt == JOptionPane.YES_OPTION) {
    Evento evento = new Evento(parametrosGestion.getEventoBaja(), elemento, this);
    controlador.procesarEvento(evento);
    }*/
}                                           

private void jButtonNuevoActionPerformed(java.awt.event.ActionEvent evt) {                                             
    int filaSeleccionada = jTableListado.getSelectedRow();
    System.out.println(filaSeleccionada);
    if (filaSeleccionada > -1) {
        System.out.println("--:" + tableModelGen.getElemento(filaSeleccionada));
        fichaElemento.fijarModelo(tableModelGen.getElemento(filaSeleccionada));
        fichaElemento.setModo("alta");
        fichaElemento.setVisible(true);
    }
}                                            
    // Variables declaration - do not modify                     
    private javax.swing.JButton jButtonAceptar;
    private javax.swing.JButton jButtonBaja;
    private javax.swing.JButton jButtonModificar;
    private javax.swing.JButton jButtonMostrar;
    private javax.swing.JButton jButtonNuevo;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JTable jTableListado;
    // End of variables declaration                   

    @Override
    public void modelPropertyChange(PropertyChangeEvent evt) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    @Override
    public void procesarEvento(Evento evento) {
        TipoEvento tipoEvento = evento.getTipoEvento();

        switch (tipoEvento) {
            case CONSULTA_GENERAL:
                //List lista = (List) evento.getInfo();
                fijarModelo((Collection) evento.getInfo());
                break;

            case ALTA_ELEMENTO_OK:
                tableModelGen.addElemento(evento.getInfo());
                break;

            case BAJA_ELEMENTO_OK:
                tableModelGen.borrarElemento(evento.getInfo());
                break;
            case MODIFICACION_ELEMENTO_OK:
               // tableModelGen.modifyElemento(evento.getInfo());
                break;

            default:
                throw new AssertionError(tipoEvento + ": Evento no controlado");
        }
    }

    @Override
    public void fijarModelo(Object object) {
    }

    // Implementacion de los metodos de 'GestionarModelo'
    @Override
    public void fijarModelo(Collection object) {
        //assert (object.getClass().getName().endsWith(".Catalogo")) : "fijarModelo: clase suminastrada invalida";

        parametrosGestion.getBaseTableModelGen().initModel(object);
        jTableListado.setModel(parametrosGestion.getBaseTableModelGen());
        jTableListado.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
    }

    @Override
    public void setDelegado(GestorEventos gestorEventosDelegado) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    @Override
    public Object obtenerModelo() {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    class MyTableModelListener implements TableModelListener {

        JTable table;

        // It is necessary to keep the table since it is not possible
        // to determine the table from the event's source
        MyTableModelListener(JTable table) {
            this.table = table;
        }

        @Override
        public void tableChanged(TableModelEvent e) {
            System.out.println("tableChanged:" + e);
            int firstRow = e.getFirstRow();
            int lastRow = e.getLastRow();
            int mColIndex = e.getColumn();

            switch (e.getType()) {
                case TableModelEvent.INSERT:
                    // The inserted rows are in the range [firstRow, lastRow]
                    for (int r = firstRow; r <= lastRow; r++) {
                        // Row r was inserted
                    }
                    break;
                case TableModelEvent.UPDATE:
                    System.out.println("tableChanged-> UPDATE");
                    //table.firefireTableChanged(e);
                    if (firstRow == TableModelEvent.HEADER_ROW) {
                        if (mColIndex == TableModelEvent.ALL_COLUMNS) {
                            // A column was added
                        } else {
                            // Column mColIndex in header changed
                        }
                    } else {
                        // The rows in the range [firstRow, lastRow] changed
                        for (int r = firstRow; r <= lastRow; r++) {
                            // Row r was changed

                            if (mColIndex == TableModelEvent.ALL_COLUMNS) {
                                // All columns in the range of rows have changed
                            } else {
                                // Column mColIndex changed
                            }
                        }
                    }
                    break;
                case TableModelEvent.DELETE:
                    // The rows in the range [firstRow, lastRow] changed
                    for (int r = firstRow; r <= lastRow; r++) {
                        // Row r was deleted
                    }
                    break;
            }
        }
    }
}


